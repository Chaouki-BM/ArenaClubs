...
const FollowingViewUser = () => {
    ...
    const loadfollwers = async () => {
        await Client.post("/getallfollowing", emailView)
            .then(function (res) {
                setfollowersV(res.data)
                ...
                <View style={{ borderRadius: 7, backgroundColor: albumS, width: 350, alignSelf: "center", marginTop: 30 }}>
                    <View style={{ borderRadius: 7, backgroundColor: albumS, width: 350, alignSelf: "center", marginTop: 20 }}>
                        <View style={{ flexDirection: row }}>
                            <Text style={{ marginTop: 15, marginHorizontal: 20, marginBottom: 15, color: maincolor, fontSize: 19 }}>{language.following}</Text>
                        </View>
                        <TextInput
                            ...
                            value={search.search}
                        />
                        {followersV.map((follower, index) => {

                            if (follower.name_followed.indexOf(search.search) != -1) { show = true }
                            else { show = false }
                                ...
                                return (
                                    <View key={index} style={{ flexDirection: 'row' }}>
                                        ...
                                        <Avatar
                                            ...
                                            //source={image}
                                            source={{ uri: `${Ip}${follower.img_followed}` }}
                                        />
                                        <Text style={{ marginVertical: 15, marginLeft: 20, color: textcoler, fontStyle: 'italic', fontSize: 17 }}>{follower.name_followed}</Text>
                                        <View style={{ flexDirection: 'row' }}>
...
export default FollowingViewUser
